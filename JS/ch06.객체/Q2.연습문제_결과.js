...
console.log(filePath.substring(index));
...
let sample = 'let sample = \`the grown-ups\' response, this time, was to advise me to lay aside my drawings of boa constrictors, whether from the inside or the outside, and devote myself instead to geography, history, arithmetic, and grammar. That is why, at the, age of six, I gave up what might have been a magnificent career as a painter. I had been disheartened by the failure of my Drawing Number One and my Drawing Number Two. Grown-ups never understand anything by themselves, and it is tiresome for children to be always and forever explaining things to the.\`;'
// 구둣점 제거
let clearStr = sample.replace(/[-',.]/g,'').toLowerCase();
console.log(clearStr);

// 배열로 만든 후, 배열의 원소가 the 면 카운트 값을 증가 시킴
let srtArr = clearStr.split(' ');
let count = 0;
for (let word of srtArr) {
    if (word === 'the') count++;
}
console.log(count);